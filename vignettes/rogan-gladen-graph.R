...
# rogan_gladen_plot(sens_n = 30, spec_n=100)
# a UAD example
...
rogan_gladen_plot = function(
  ...
  app2 = ggplot(tmp, aes(x=prev,y=ap*size))+
    ...
    scale_y_continuous(sec.axis = sec_axis( trans=~./size, name="apparent prevalence (AP)", breaks=round(unique(marks$ap_mark)*1000)/1000), expand = c(0, 0))+
    scale_x_continuous(expand = c(0, 0), sec.axis = sec_axis( trans=~.*size, name=sprintf("disease count (N=%d)",size), breaks = round(unique(marks$p_mark*size))))+
  ...
  mx = ggplot(marginal_x, aes(x=prev, y=p_test, colour=as.character(test),fill=as.character(test)))+geom_area(position=position_identity(),alpha=0.1)+
    ...
    scale_x_continuous(expand = c(0, 0), name="true prevalence (P)")+scale_y_reverse(expand = c(0.005, 0.005),breaks = NULL)+.gg_hide_Y_axis()+
    theme(
  ...
  my = ggplot(marginal_y, aes(x=test, y=p_test, colour=as.character(prev),fill=as.character(prev)))+
    ...
    ggplot2::scale_x_continuous(expand = c(0, 0), name=sprintf("test positive count (N=%d)",size))+
    coord_flip(clip="off")+scale_y_reverse(expand = c(0, 0.005),breaks = NULL)+.gg_hide_X_axis()+
    theme(
