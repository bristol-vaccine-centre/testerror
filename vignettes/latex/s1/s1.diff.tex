...
%DIF DEL s1.first_submission.tex   Fri Dec  8 10:43:11 2023
%DIF ADD s1.tex                    Tue Mar 19 12:59:00 2024
...
The likelihood that any given person is actual case positive and observed test positive is the true positives \DIFaddbegin \DIFadd{(}\DIFaddend \(TP\)\DIFaddbegin \DIFadd{)}\DIFaddend , and can be defined in terms of the sensitivity and prevalence (\(sens\) and \(prev\)). \DIFaddbegin \DIFadd{Similarly we can define false positives (\(FP\)), false negatives (\(FN\)) and true negatives (\(FN\)).
}\DIFaddend 
...
This alternative expression is an estimator for panel \DIFdelbegin \DIFdel{specificity }\DIFdelend \DIFaddbegin \DIFadd{sensitivity }\DIFaddend based on observed data, but as with the Rogan-Gladen estimate for true prevalence, it may become inaccurate when the observations of component test positivity is a poor proxy of expected apparent prevalence (\ie outside of the range \((1-spec_n) < \widehat{AP_n} < sens_n\)). However, in practice, this is less of an issue than the Rogan-Gladen estimator, and truncation is not required to produce reasonable estimates of combined sensitivity\DIFaddbegin \DIFadd{, due to the Rogan-Gladen term }\eqref{eq:rogan-gladen} \DIFadd{being in both numerator and denominator}\DIFaddend .

\subsection{Panel prevalence from data}

These expressions allow us to create an estimator \DIFaddbegin \DIFadd{for }\DIFaddend the ``actual'' prevalence of a condition using the observed test positivity of a panel of component tests by applying the Rogan-Gladen estimator. This can be calculated from raw component test data, and component test sensitivity and specificity:
...
Whilst the characterisation is helpful to test the estimator in a wide range of scenarios, we resort to multiple simulations for further detailed characterisation. Following the model described in \eqref{eq:simulation-setup}, we run 8 batches of 100 simulations. Each simulation consisting of between 2 and 9 components\DIFaddbegin \DIFadd{, \(N_{sim}\) }\DIFaddend (depending on the batch) and each component in every simulation has a randomly assigned, but generally plausible, prevalence, sensitivity and specificity. Each component is given a random zero-inflated \DIFdelbegin \DIFdel{prevalence (\(prev_n\)}\DIFdelend \DIFaddbegin \DIFadd{frequency, (\(freq_n\)}\DIFaddend ) using a product of two Poisson distributed quantities, \DIFdelbegin \DIFdel{constrained so that the total prevalence matches a simulation parameter }\DIFdelend \DIFaddbegin \DIFadd{which is scaled by an empirically determined factor (\(scale\)) to enforce the component prevalences combine to the simulation total prevalence }\DIFaddend (\(prev_{N_{sim}}\)) \DIFaddbegin \DIFadd{with the assumption of independence}\DIFaddend . 10000 cases are generated for each of the 800 simulations with ``actual'' subtype status depending on the component prevalence, and a test observation for each subtype depending on the ``actual'' subtype status, sensitivity and specificity. ``Actual'' combined disease status and observed test panel status are calculated for each of the 10000 cases. The model for each simulation is described below:
...
prev\DIFdelbegin \DIFdel{_N }\DIFdelend \DIFaddbegin \DIFadd{_{N_{sim}} }\DIFaddend &\sim Beta(2,10),\\
\DIFdelbegin \DIFdel{dist}\DIFdelend \DIFaddbegin \DIFadd{freq}\DIFaddend _n &\sim Poisson(5) \times Poisson(1),\\
prev_n &= \DIFdelbegin \DIFdel{\frac{dist_n}{\sum{dist_n}} \times prev_{N_{sim}} }\DIFdelend \DIFaddbegin \DIFadd{\frac{scale \times freq_n}{\sum{freq_n}}, }\\
\DIFadd{prev_{N_{sim}} }&\DIFadd{= 1-\prod_{n \in N}{\big(1-prev_n}}\big)\DIFaddend , \\
k &\in \{1,2,\dots,10000\},\\
I(A_{n,k}) &\sim Bernoulli(prev_n),\\
I(O_{n,k}) &\sim Bernoulli((1-spec_n) \times (1-I(A_{n,k})) + sens_n \times I(A_{n,k})),\\
I(A_{N,k}) &= \DIFdelbegin \DIFdel{\prod_n{I(A_{n,k})}}\DIFdelend \DIFaddbegin \DIFadd{1-\prod_n{\big(1-I(A_{n,k})\big)}}\DIFaddend ,\\
I(O_{N,k}) &= \DIFdelbegin \DIFdel{\prod_n{I(O_{n,k})}}\DIFdelend \DIFaddbegin \DIFadd{1-\prod_n{\big(1-I(O_{n,k})\big)}}\DIFaddend .\\
\end{aligned}
\mytag{eq:simulation-setup}
\end{equation*}

\DIFdelbegin \DIFdel{For each simulation, the 10000 combined panel test status and }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Panel prevalence validation}}

\DIFadd{Using the simulated cases we estimate }\DIFaddend the ``actual'' \DIFdelbegin \DIFdel{combined disease status are aggregated to provide a per-simulation estimate of panel sensitivity (\(\widehat{sens_{N,sim}}\))and panel specificity (\(\widehat{spec_{N,sim}}\)), including confidence intervals, as defined in }%DIFDELCMD < \eqref{eq:simulation-output}%%%
\DIFdelend \DIFaddbegin \DIFadd{prevalence of each component in the simulation using the proportion of test positives for each component (\(\widehat{AP_{n,sim}}\)), the simulation parameters for sensitivity and specificity and the Rogan-Gladen estimator }\eqref{eq:rogan-gladen}\DIFadd{. We compare this to the ``actual'' prevalence observed in the simulation as the proportion of simulation disease positives (\(\overline{prev_{n,sim}}\))}\DIFaddend :

\begin{equation*}
\begin{aligned}
\DIFdelbegin \DIFdel{TP_{N} }%DIFDELCMD < &%%%
\DIFdel{= \sum_i{I(A_{N,i}) \times I(O_{N,i})}, }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{FP_{N} }%DIFDELCMD < &%%%
\DIFdel{= \sum_i{I(\neg A_{N,i}) \times I(O_{N,i})}, }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{FN_{N} }%DIFDELCMD < &%%%
\DIFdel{= \sum_i{I(A_{N,i}) \times I(\neg O_{N,i})}, }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{TN_{N} }%DIFDELCMD < &%%%
\DIFdel{= \sum_i{I(\neg A_{N,i}) \times I(\neg O_{N,i})}, }%DIFDELCMD < \\
%DIFDELCMD < \widehat{spec_{N,sim}} %%%
\DIFdelend \DIFaddbegin \widehat{AP_{n,sim}} \DIFaddend &= \DIFdelbegin \DIFdel{\frac{TN_{N}}{FP_{N}+TN_{N}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{|k|}\sum_k{I(O_{n,k})}}\DIFaddend , \\
\DIFdelbegin %DIFDELCMD < \widehat{sens_{N,sim}} %%%
\DIFdelend \DIFaddbegin \widehat{prev_{n,sim}} \DIFaddend &= \DIFdelbegin \DIFdel{\frac{TP_{N}}{FN_{N}+TP_{N}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{
    \widehat{AP_{n,sim}} + spec_n -1
  }{
    sens_n + spec_n - 1
  }}\DIFaddend ,\\
\DIFdelbegin %DIFDELCMD < \widehat{AP_{N,sim}} %%%
\DIFdelend \DIFaddbegin \overline{prev_{n,sim}} \DIFaddend &= \DIFdelbegin \DIFdel{\frac{1}{10000}\sum_i{I(O_{N,i})}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{|k|}\sum_k{I(A_{n,k})}}\DIFaddend .
\end{aligned}
\DIFdelbegin %DIFDELCMD < \mytag{eq:simulation-output}
%DIFDELCMD < %%%
\DIFdelend \end{equation*}

\DIFdelbegin \DIFdel{These per-simulation values are compared to the theoretical values of panel specificity (\(spec_N\)) }%DIFDELCMD < \eqref{eq:spec-theory}%%%
\DIFdel{, to the theoretical panel sensitivity derived from the simulation parameter ``actual'' prevalence (\(sens_N\)) obtained using }%DIFDELCMD < \eqref{eq:sens-theory}  %%%
\DIFdel{and to the estimated panel sensitivity  (\(\widehat{sens_N}\)) derived from ``observed'' simulation component test apparent prevalence, obtained using }%DIFDELCMD < \eqref{eq:sens-estimator}%%%
\DIFdel{. Lastly we can also estimate the ``actual'' }\DIFdelend \DIFaddbegin \DIFadd{We also use the simulated cases to estimate the }\DIFaddend prevalence of the overall disease by using the Rogan-Gladen estimator \DIFaddbegin \eqref{eq:rogan-gladen}\DIFaddend , observed panel test \DIFdelbegin \DIFdel{positives}\DIFdelend \DIFaddbegin \DIFadd{positivity (\(\widehat{AP_{N,sim}}\))}\DIFaddend , predicted panel sensitivity \DIFdelbegin \DIFdel{and specificity}\DIFdelend \DIFaddbegin \eqref{eq:sens-theory} \DIFadd{and specificity }\eqref{eq:spec-theory}\DIFaddend . This can be directly compared to the \DIFdelbegin \DIFdel{parameterised }\DIFdelend \DIFaddbegin \DIFadd{``actual'' }\DIFaddend simulation panel prevalence\DIFaddbegin \DIFadd{, as the proportion of simulation disease positives (\(\overline{prev_{N,sim}}\))}\DIFaddend :

\begin{equation*}
\begin{aligned}
\DIFaddbegin \widehat{AP_{N,sim}} &\DIFadd{= \frac{1}{|k|}\sum_k{I(O_{N,k})}, }\\
\DIFaddend \widehat{prev_{N,sim}} &= \frac{
    \widehat{AP_{N,sim}} + spec_N -1
  }{
    \widehat{sens_N} + spec_N - 1
  }\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, }\\
\overline{prev_{N,sim}} &\DIFadd{= \frac{1}{|k|}\sum_k{I(A_{N,k})}. }\\
\DIFaddend \end{aligned}
\DIFdelbegin %DIFDELCMD < \mytag{eq:simulation-output}
%DIFDELCMD < %%%
\DIFdelend \end{equation*}

The primary results of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend simulation are shown in figure \ref{fig:A3}. \DIFdelbegin \DIFdel{Panel }\DIFdelend \DIFaddbegin \DIFadd{Subfigure }\DIFaddend A shows the relationship between the \DIFaddbegin \DIFadd{apparent prevalence, i.e. the test positivity rate in the simulation, on the y-axis (\(\widehat{AP_{n,sim}}\)) and the true }\DIFaddend prevalence of each component\DIFdelbegin \DIFdel{disease as a provided simulationparameter and we }\DIFdelend \DIFaddbegin \DIFadd{, i.e. the disease positivity rate within the simulation, on the x-axis (\(\overline{prev_{n,sim}}\)). We }\DIFaddend see the effect of false positives in low prevalence, biasing the apparent prevalence high with respect to the actual value. In \DIFdelbegin \DIFdel{panel }\DIFdelend \DIFaddbegin \DIFadd{subfigure }\DIFaddend B, the \DIFdelbegin \DIFdel{components are }\DIFdelend \DIFaddbegin \DIFadd{component apparent prevalence is }\DIFaddend adjusted using the Rogan-Gladen estimator, using simulation parameters for sensitivity and specificity \DIFdelbegin \DIFdel{correcting the bias}\DIFdelend \DIFaddbegin \DIFadd{to produce an unbiased estimate of prevalence (\(\widehat{prev_{n,sim}}\): y-axis) which agrees well with the true prevalence. This is the usual single test application of Rogan-Gladen, and shown here for comparison}\DIFaddend .

Panel C shows the relationship between the apparent prevalence of the combined panel result\DIFaddbegin \DIFadd{, i.e. the test positivity rate of the combination, (\(\widehat{AP_{N,sim}}\): y-axis) }\DIFaddend compared to the \DIFdelbegin \DIFdel{actual }\DIFdelend prevalence of the combined disease\DIFdelbegin \DIFdel{as a provided simulation parameter}\DIFdelend \DIFaddbegin \DIFadd{, i.e. the disease positivity rate in the simulation (\(\overline{prev_{N,sim}}\): x-axis)}\DIFaddend . The scale of the error \DIFdelbegin \DIFdel{can be quite large }\DIFdelend \DIFaddbegin \DIFadd{using the apparent prevalence as an estimator is quite large compared to that seen in subfigure A, }\DIFaddend as the panel prevalence is compounding multiple false positive errors from each component\DIFdelbegin \DIFdel{, depending on the sensitivity and specificity of the components}\DIFdelend . The scale of the error will depend on the number of components in the panel and the sensitivities and specificities of the components as \DIFdelbegin \DIFdel{described }\DIFdelend \DIFaddbegin \DIFadd{characterised }\DIFaddend above. In \DIFdelbegin \DIFdel{Panel }\DIFdelend \DIFaddbegin \DIFadd{subfigure }\DIFaddend D, the \DIFaddbegin \DIFadd{simple }\DIFaddend Rogan-Gladen correction is applied \DIFdelbegin \DIFdel{, using the panel apparent prevalence, and the }\DIFdelend \DIFaddbegin \eqref{eq:rogan-gladen} \DIFadd{to the apparent panel prevalence, using }\DIFaddend estimates of panel sensitivity and specificity from this analysis (equations \eqref{eq:sens-estimator} and  \eqref{eq:spec-theory}). The correction is effective in restoring an accurate estimate of the actual prevalence \DIFaddbegin \DIFadd{(\(\widehat{prev_{N,sim}}\): y-axis), giving us confidence our estimates of panel sensitivity and specificity are valid, and that they can be used to create unbiased estimates of panel prevalence}\DIFaddend .

\begin{figure}[h!]
\centering
  \includegraphics{fig/qq-prevalence-prediction-v-simulation.png}
  \caption{Apparent prevalence (uncorrected estimate) and Rogan-Gladen (corrected estimate) as compared to simulation prevalence. \DIFdelbeginFL \DIFdelFL{Panels }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The left column compares unadjusted apparent prevalence (\(\widehat{AP_{N,sim}}\)), as an estimate of prevalence, to simulation disease positive rate (\(\overline{prev_{N,sim}}\)), as actual prevalence. The right column compares the Rogan-Gladen modelled prevalence estimate (\(\widehat{prev_{N,sim}}\)) to the same simulation disease positive rate (actual prevalence). Subfigures }\DIFaddendFL A and B show results each of the 4400 single component tests \DIFdelbeginFL \DIFdelFL{in the 800 simulations (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with }\DIFaddendFL various component test sensitivities \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL specificities \DIFaddbeginFL \DIFaddFL{(100 each of 8 blocks with each block containing 2-9 component panels}\DIFaddendFL ). \DIFdelbeginFL \DIFdelFL{Panels }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Subfigures }\DIFaddendFL C and D show the results of the 800 combined panels (2-9 components per panel, various component distributions).}
\label{fig:A3}
\end{figure}

\DIFaddbegin \subsection{\DIFadd{Panel sensitivity and specificity validation}}

\DIFadd{Further validation of panel sensitivity and specificity was performed. For each of 800 simulations, the 10000 combined panel test status and the ``actual'' combined disease status are aggregated to provide a per-simulation estimate of panel sensitivity (\(\widehat{sens_{N,sim}}\)) and panel specificity (\(\widehat{spec_{N,sim}}\)), as defined in }\eqref{eq:simulation-output}\DIFadd{, with binomial confidence intervals on panel sensitivity and specificity calculated using Wilson's method \mbox{%DIFAUXCMD
\cite{brown2001}}\hspace{0pt}%DIFAUXCMD
:
}

\begin{equation*}
\begin{aligned}
\DIFadd{TP_{N} }&\DIFadd{= \sum_k{I(A_{N,k}) \times I(O_{N,k})}, }\\
\DIFadd{FP_{N} }&\DIFadd{= \sum_k{I(\neg A_{N,k}) \times I(O_{N,k})}, }\\
\DIFadd{FN_{N} }&\DIFadd{= \sum_k{I(A_{N,k}) \times I(\neg O_{N,k})}, }\\
\DIFadd{TN_{N} }&\DIFadd{= \sum_k{I(\neg A_{N,k}) \times I(\neg O_{N,k})}, }\\
\widehat{spec_{N,sim}} &\DIFadd{= \frac{TN_{N}}{FP_{N}+TN_{N}}, }\\
\widehat{sens_{N,sim}} &\DIFadd{= \frac{TP_{N}}{FN_{N}+TP_{N}}, }\\
\end{aligned}
\mytag{eq:simulation-output}
\end{equation*}

\DIFadd{These per-simulation observed values for sensitivity and specificity are compared to the predicted theoretical values of panel specificity (\(spec_N\)) from }\eqref{eq:spec-theory}\DIFadd{; to the theoretical panel sensitivity derived from the simulation parameter ``actual'' prevalence (\(sens_N\)) obtained using }\eqref{eq:sens-theory}\DIFadd{; and to the estimated panel sensitivity  (\(\widehat{sens_N}\)) derived from ``observed'' simulation component test apparent prevalence (\(\widehat{AP_{n,sim}}\)) obtained using }\eqref{eq:sens-estimator}\DIFadd{.
}

\DIFaddend In panel A, figure \ref{fig:A4}, we also see a close relationship between the theoretical panel specificity and panel specificity \DIFdelbegin \DIFdel{estimated }\DIFdelend \DIFaddbegin \DIFadd{observed }\DIFaddend from each simulation scenario. As specificity is estimated from test negative cases the confidence intervals of the simulation \DIFdelbegin \DIFdel{estimates }\DIFdelend \DIFaddbegin \DIFadd{observations }\DIFaddend are quite narrow. \DIFaddbegin \DIFadd{No further validation of specificity was performed.
}\DIFaddend 

\begin{figure}[h!]
\centering
  \includegraphics{fig/qq-prediction-v-simulation.png}
  \caption{Panel sensitivity predicted using \eqref{eq:spec-theory} (subfigure A) and specificity predicted using equations \eqref{eq:sens-theory} (subfigure B) and equation \eqref{eq:sens-estimator} - (subfigure C) as compared to panel sensitivity and specificity values derived from the  combined test positive and actual positives from simulated component data. \DIFaddbeginFL \DIFaddFL{Magenta points highlight those simulations whose confidence limits do not include the theoretical value.}\DIFaddendFL }
\label{fig:A4}
\end{figure}

\DIFdelbegin \DIFdel{Panel }\DIFdelend \DIFaddbegin \DIFadd{Subfigure }\DIFaddend B shows a comparison between predicted value of panel sensitivity (using \eqref{eq:sens-theory} in this formal analysis) and the observed panel sensitivity from simulation. \DIFdelbegin \DIFdel{Panel }\DIFdelend \DIFaddbegin \DIFadd{Subfigure }\DIFaddend C shows panel sensitivity predicted from component test positive rates that would be observed in a real setting (\eqref{eq:sens-estimator} in this analysis). In both cases there is a reasonable agreement between predicted and simulated values. \DIFaddbegin \DIFadd{Further analysis was performed on the error between simulation observed and theory predicted sensitivity.
}\DIFaddend 

\begin{figure}[h!]
\centering
  \includegraphics{fig/error-prediction-v-simulation.png}
  \caption{Error between panel sensitivity estimates predicted using equations \eqref{eq:sens-theory} (top row) and equation \eqref{eq:sens-estimator} (bottom row) and observed simulation panel sensitivity. Error size is plotted as a function of simulation panel prevalence (first row), panel specificity (second row) and number of panel components (third row). Error magnitude depends on the quality of the estimator and the inherent error from using simulated data, which is higher when prevalence is low. \DIFaddbeginFL \DIFaddFL{Magenta points highlight those simulations whose confidence limits do not include the theoretical value.}\DIFaddendFL }
\label{fig:A5}
\end{figure}
