...
\begin{equation*}
\begin{aligned}
I(A_{N,k}) = 1-\prod_{n \in N}{(1-I(A_{n,k}))} \\
\end{aligned}
\end{equation*}

Where \(I\) is the indicator function and a value of one represents presence and zero is absence, and \(A_{n,k}\) describes the actual status of the disease subtype \(n\) in an individual \(k\).

Such a scenario may occur when considering a single disease, that may be caused by multiple distinct pathogens which are tested for separately, and could, at least in theory, occur together.
...
The likelihood that any given person is actual case positive and observed test positive is the true positives (\(TP\)), and can be defined in terms of the sensitivity and prevalence (\(sens\) and \(prev\)). Similarly we can define false positives (\(FP\)), false negatives (\(FN\)) and true negatives (\(FN\)).
...
1-P(A_N) &= \prod_{n \in N}{\big(1-P(A_n)\big)}, \\
...
\begin{equation*}
\begin{aligned}
P(O_N) &= 1-\prod_{n \in N}{\big(1-P(O_n)\big)} \\
 &= 1-\prod_{n \in N}{P(\neg O_n)} \\
...
This alternative expression is an estimator for panel sensitivity based on observed data, but as with the Rogan-Gladen estimate for true prevalence, it may become inaccurate when the observations of component test positivity is a poor proxy of expected apparent prevalence (\ie outside of the range \((1-spec_n) < \widehat{AP_n} < sens_n\)). However, in practice, this is less of an issue than the Rogan-Gladen estimator, and truncation is not required to produce reasonable estimates of combined sensitivity, due to the Rogan-Gladen term \eqref{eq:rogan-gladen} being in both numerator and denominator.

\subsection{Panel prevalence from data}

These expressions allow us to create an estimator for the ``actual'' prevalence of a condition using the observed test positivity of a panel of component tests by applying the Rogan-Gladen estimator. This can be calculated from raw component test data, and component test sensitivity and specificity:
...
Whilst the characterisation is helpful to test the estimator in a wide range of scenarios, we resort to multiple simulations for further detailed characterisation. Following the model described in \eqref{eq:simulation-setup}, we run 8 batches of 100 simulations. Each simulation consisting of between 2 and 9 components, \(N_{sim}\) (depending on the batch) and each component in every simulation has a randomly assigned, but generally plausible, prevalence, sensitivity and specificity. Each component is given a random zero-inflated frequency, (\(freq_n\)) using a product of two Poisson distributed quantities, which is scaled by an empirically determined factor (\(scale\)) to enforce the component prevalences combine to the simulation total prevalence (\(prev_{N_{sim}}\)) with the assumption of independence. 10000 cases are generated for each of the 800 simulations with ``actual'' subtype status depending on the component prevalence, and a test observation for each subtype depending on the ``actual'' subtype status, sensitivity and specificity. ``Actual'' combined disease status and observed test panel status are calculated for each of the 10000 cases. The model for each simulation is described below:
...
prev_{N_{sim}} &\sim Beta(2,10),\\
freq_n &\sim Poisson(5) \times Poisson(1),\\
prev_n &= \frac{scale \times freq_n}{\sum{freq_n}}, \\
prev_{N_{sim}} &= 1-\prod_{n \in N}{\big(1-prev_n}\big), \\
k &\in \{1,2,\dots,10000\},\\
I(A_{n,k}) &\sim Bernoulli(prev_n),\\
I(O_{n,k}) &\sim Bernoulli((1-spec_n) \times (1-I(A_{n,k})) + sens_n \times I(A_{n,k})),\\
I(A_{N,k}) &= 1-\prod_n{\big(1-I(A_{n,k})\big)},\\
I(O_{N,k}) &= 1-\prod_n{\big(1-I(O_{n,k})\big)}.\\
\end{aligned}
\mytag{eq:simulation-setup}
\end{equation*}

\subsection{Panel prevalence validation}

Using the simulated cases we estimate the ``actual'' prevalence of each component in the simulation using the proportion of test positives for each component (\(\widehat{AP_{n,sim}}\)), the simulation parameters for sensitivity and specificity and the Rogan-Gladen estimator \eqref{eq:rogan-gladen}. We compare this to the ``actual'' prevalence observed in the simulation as the proportion of simulation disease positives (\(\overline{prev_{n,sim}}\)):

\begin{equation*}
\begin{aligned}
\widehat{AP_{n,sim}} &= \frac{1}{|k|}\sum_k{I(O_{n,k})}, \\
\widehat{prev_{n,sim}} &= \frac{
    \widehat{AP_{n,sim}} + spec_n -1
  }{
    sens_n + spec_n - 1
  },\\
\overline{prev_{n,sim}} &= \frac{1}{|k|}\sum_k{I(A_{n,k})}.
\end{aligned}
\end{equation*}

We also use the simulated cases to estimate the prevalence of the overall disease by using the Rogan-Gladen estimator \eqref{eq:rogan-gladen}, observed panel test positivity (\(\widehat{AP_{N,sim}}\)), predicted panel sensitivity \eqref{eq:sens-theory} and specificity \eqref{eq:spec-theory}. This can be directly compared to the ``actual'' simulation panel prevalence, as the proportion of simulation disease positives (\(\overline{prev_{N,sim}}\)):
...
The primary results of this simulation are shown in figure \ref{fig:A3}. Subfigure A shows the relationship between the apparent prevalence, i.e. the test positivity rate in the simulation, on the y-axis (\(\widehat{AP_{n,sim}}\)) and the true prevalence of each component, i.e. the disease positivity rate within the simulation, on the x-axis (\(\overline{prev_{n,sim}}\)). We see the effect of false positives in low prevalence, biasing the apparent prevalence high with respect to the actual value. In subfigure B, the component apparent prevalence is adjusted using the Rogan-Gladen estimator, using simulation parameters for sensitivity and specificity to produce an unbiased estimate of prevalence (\(\widehat{prev_{n,sim}}\): y-axis) which agrees well with the true prevalence. This is the usual single test application of Rogan-Gladen, and shown here for comparison.

Panel C shows the relationship between the apparent prevalence of the combined panel result, i.e. the test positivity rate of the combination, (\(\widehat{AP_{N,sim}}\): y-axis) compared to the prevalence of the combined disease, i.e. the disease positivity rate in the simulation (\(\overline{prev_{N,sim}}\): x-axis). The scale of the error using the apparent prevalence as an estimator is quite large compared to that seen in subfigure A, as the panel prevalence is compounding multiple false positive errors from each component. The scale of the error will depend on the number of components in the panel and the sensitivities and specificities of the components as characterised above. In subfigure D, the simple Rogan-Gladen correction is applied \eqref{eq:rogan-gladen} to the apparent panel prevalence, using estimates of panel sensitivity and specificity from this analysis (equations \eqref{eq:sens-estimator} and  \eqref{eq:spec-theory}). The correction is effective in restoring an accurate estimate of the actual prevalence (\(\widehat{prev_{N,sim}}\): y-axis), giving us confidence our estimates of panel sensitivity and specificity are valid, and that they can be used to create unbiased estimates of panel prevalence.

\begin{figure}[h!]
\centering
  \includegraphics{fig/qq-prevalence-prediction-v-simulation.png}
  \caption{Apparent prevalence (uncorrected estimate) and Rogan-Gladen (corrected estimate) as compared to simulation prevalence. The left column compares unadjusted apparent prevalence (\(\widehat{AP_{N,sim}}\)), as an estimate of prevalence, to simulation disease positive rate (\(\overline{prev_{N,sim}}\)), as actual prevalence. The right column compares the Rogan-Gladen modelled prevalence estimate (\(\widehat{prev_{N,sim}}\)) to the same simulation disease positive rate (actual prevalence). Subfigures A and B show results each of the 4400 single component tests with various component test sensitivities and specificities (100 each of 8 blocks with each block containing 2-9 component panels). Subfigures C and D show the results of the 800 combined panels (2-9 components per panel, various component distributions).}
\label{fig:A3}
\end{figure}

\subsection{Panel sensitivity and specificity validation}

Further validation of panel sensitivity and specificity was performed. For each of 800 simulations, the 10000 combined panel test status and the ``actual'' combined disease status are aggregated to provide a per-simulation estimate of panel sensitivity (\(\widehat{sens_{N,sim}}\)) and panel specificity (\(\widehat{spec_{N,sim}}\)), as defined in \eqref{eq:simulation-output}, with binomial confidence intervals on panel sensitivity and specificity calculated using Wilson's method \cite{brown2001}:

\begin{equation*}
\begin{aligned}
TP_{N} &= \sum_k{I(A_{N,k}) \times I(O_{N,k})}, \\
FP_{N} &= \sum_k{I(\neg A_{N,k}) \times I(O_{N,k})}, \\
FN_{N} &= \sum_k{I(A_{N,k}) \times I(\neg O_{N,k})}, \\
TN_{N} &= \sum_k{I(\neg A_{N,k}) \times I(\neg O_{N,k})}, \\
...
These per-simulation observed values for sensitivity and specificity are compared to the predicted theoretical values of panel specificity (\(spec_N\)) from \eqref{eq:spec-theory}; to the theoretical panel sensitivity derived from the simulation parameter ``actual'' prevalence (\(sens_N\)) obtained using \eqref{eq:sens-theory}; and to the estimated panel sensitivity  (\(\widehat{sens_N}\)) derived from ``observed'' simulation component test apparent prevalence (\(\widehat{AP_{n,sim}}\)) obtained using \eqref{eq:sens-estimator}.

In panel A, figure \ref{fig:A4}, we also see a close relationship between the theoretical panel specificity and panel specificity observed from each simulation scenario. As specificity is estimated from test negative cases the confidence intervals of the simulation observations are quite narrow. No further validation of specificity was performed.

\begin{figure}[h!]
\centering
  \includegraphics{fig/qq-prediction-v-simulation.png}
  \caption{Panel sensitivity predicted using \eqref{eq:spec-theory} (subfigure A) and specificity predicted using equations \eqref{eq:sens-theory} (subfigure B) and equation \eqref{eq:sens-estimator} - (subfigure C) as compared to panel sensitivity and specificity values derived from the  combined test positive and actual positives from simulated component data. Magenta points highlight those simulations whose confidence limits do not include the theoretical value.}
\label{fig:A4}
\end{figure}

Subfigure B shows a comparison between predicted value of panel sensitivity (using \eqref{eq:sens-theory} in this formal analysis) and the observed panel sensitivity from simulation. Subfigure C shows panel sensitivity predicted from component test positive rates that would be observed in a real setting (\eqref{eq:sens-estimator} in this analysis). In both cases there is a reasonable agreement between predicted and simulated values. Further analysis was performed on the error between simulation observed and theory predicted sensitivity.

\begin{figure}[h!]
\centering
  \includegraphics{fig/error-prediction-v-simulation.png}
  \caption{Error between panel sensitivity estimates predicted using equations \eqref{eq:sens-theory} (top row) and equation \eqref{eq:sens-estimator} (bottom row) and observed simulation panel sensitivity. Error size is plotted as a function of simulation panel prevalence (first row), panel specificity (second row) and number of panel components (third row). Error magnitude depends on the quality of the estimator and the inherent error from using simulated data, which is higher when prevalence is low. Magenta points highlight those simulations whose confidence limits do not include the theoretical value.}
\label{fig:A5}
\end{figure}
...
If the theoretical values of sensitivity are accurate then it should lie within binomial confidence limits of the simulation estimates 95\% of the time. Disagreement occurs when the theoretical value is outside the confidence interval. In figure \ref{fig:A6} we see the rates of disagreement as a function of panel prevalence, specificity and number of components. This demonstrates that in the areas of simulation parameter space where we have sufficient coverage, disagreement rates between simulated and theoretical values are close to the 0.05 value resulting from the 95\% confidence limits in the simulation. There is no clear trend.

\begin{figure}[h!]
\centering
  \includegraphics{fig/calibration-prediction-v-simulation.png}
  \caption{Percentage disagreement between panel sensitivity estimates predicted using equations \eqref{eq:sens-theory} (top row) and equation \eqref{eq:sens-estimator} (bottom row) versus observed simulation panel sensitivity. Disagreement accounts for uncertainty introduced by simulation by comparing the binomial confidence intervals of simulation panel sensitivity estimates with sensitivity predictions. 95\% confidence limits imply disagreement should be at the level of 5\%. Disagreement rates are plotted as a function of simulation panel prevalence (first column), panel specificity (second column) and number of panel components (third column).}
\label{fig:A6}
\end{figure}
