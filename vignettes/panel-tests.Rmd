...
```{r}
tmp = testerror:::panel_example(comp_spec = 0.995, comp_sens = 0.8, n_comp = 20, n_samples = 2000, n_controls = 800,exact = FALSE)
design = tmp$design %>% dplyr::filter(test != "Panel")
design %>% dplyr::mutate(across(-test, ~sprintf("%1.2f%%", .x*100))) %>%
  default_table()
```
...
testerror:::demo_bar_plot(tmp$summary %>% dplyr::inner_join(bind_rows(corr,corr2,corr3),by="test"))
```
