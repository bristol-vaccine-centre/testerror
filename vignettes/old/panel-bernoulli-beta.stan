// Use this when you have prior estimates for sensitivity and specificity
// for all individual components of the test
data {
  int<lower = 1> n_test;
  ...
  // specificity priors based on expert opinion or previous studies
  real<lower = 0> fp_spec[n_test];
  real<lower = 0> tn_spec[n_test];
  
  // specificity priors based on expert opinion or previous studies
  real<lower = 0> fn_sens[n_test];
  real<lower = 0> tp_sens[n_test];
...
model {
  ...
  for (n in 1:n_test) {
    ...
    target += beta_lpdf(spec[n] | tn_spec[n], fp_spec[n]);
    target += beta_lpdf(sens[n] | tp_sens[n], fn_sens[n]);
