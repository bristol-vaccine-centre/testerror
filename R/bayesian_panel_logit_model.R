...
#'
#' The beta distribution priors in this model will actually be converted to
#' logit_normal distributions
#'
...
#' @export
bayesian_panel_logit_model = function(
    panel_pos_obs,
    panel_n_obs,
    pos_obs,
    n_obs,
  ...
  # standata$logit_mu_p_prior = logit(ep)
  # standata$logit_sigma_p_prior = abs(logit(ep))/2
  ...
  result = .summarise_stan_results(
    fit_combined, test_names, panel_name, confint, "bayes (logit)", fmt)
